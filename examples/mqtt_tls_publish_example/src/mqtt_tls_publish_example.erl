...
-module(mqtt_tls_publish_example).
...
-define(MESSAGE, <<"Hello World over TLS">>).
...
run() ->
    %% TLS keeps the hostname so SNI works (auto-populated by ssl:connect/3).
    io:format("connecting to ~s:~p (TLS)~n", [?BROKER, ?TLS_PORT]),
