...
encode_remaining_length_0_test() ->
    ?assertEqual(<<0>>, mqtt_proto:encode_remaining_length(0)).

encode_remaining_length_127_test() ->
    ?assertEqual(<<127>>, mqtt_proto:encode_remaining_length(127)).

encode_remaining_length_128_test() ->
    ?assertEqual(<<128, 1>>, mqtt_proto:encode_remaining_length(128)).

encode_remaining_length_16383_test() ->
    ?assertEqual(<<16#FF, 16#7F>>, mqtt_proto:encode_remaining_length(16383)).

encode_remaining_length_16384_test() ->
    ?assertEqual(<<16#80, 16#80, 1>>, mqtt_proto:encode_remaining_length(16384)).

encode_remaining_length_2097151_test() ->
    ?assertEqual(<<16#FF, 16#FF, 16#7F>>, mqtt_proto:encode_remaining_length(2097151)).

encode_remaining_length_2097152_test() ->
    ?assertEqual(<<16#80, 16#80, 16#80, 1>>, mqtt_proto:encode_remaining_length(2097152)).

encode_remaining_length_max_test() ->
    ?assertEqual(<<16#FF, 16#FF, 16#FF, 16#7F>>, mqtt_proto:encode_remaining_length(268435455)).

decode_remaining_length_single_byte_test() ->
    ?assertEqual({ok, 0, <<>>}, mqtt_proto:decode_remaining_length(<<0>>)),
    ?assertEqual({ok, 127, <<>>}, mqtt_proto:decode_remaining_length(<<127>>)).

decode_remaining_length_two_bytes_test() ->
    ?assertEqual({ok, 128, <<>>}, mqtt_proto:decode_remaining_length(<<128, 1>>)),
    ?assertEqual({ok, 16383, <<>>}, mqtt_proto:decode_remaining_length(<<16#FF, 16#7F>>)).

decode_remaining_length_with_trailing_data_test() ->
    ?assertEqual({ok, 128, <<99>>}, mqtt_proto:decode_remaining_length(<<128, 1, 99>>)).

decode_remaining_length_incomplete_test() ->
    ?assertEqual({error, incomplete}, mqtt_proto:decode_remaining_length(<<>>)),
    ?assertEqual({error, incomplete}, mqtt_proto:decode_remaining_length(<<128>>)),
    ?assertEqual({error, incomplete}, mqtt_proto:decode_remaining_length(<<128, 128>>)).

remaining_length_roundtrip_test() ->
    Ns = [0, 1, 127, 128, 255, 16383, 16384, 2097151, 2097152, 268435455],
    lists:foreach(
        fun(N) ->
            Encoded = mqtt_proto:encode_remaining_length(N),
            ?assertEqual({ok, N, <<>>}, mqtt_proto:decode_remaining_length(Encoded))
        end,
        Ns
    ).
...
encode_minimal_connect_test() ->
    Packet = mqtt_proto:encode_connect(#{client_id => <<"test">>}),
    {ok, {connect, Data}, <<>>} = mqtt_proto:decode(Packet),
    ?assertEqual(<<"test">>, maps:get(client_id, Data)),
    ?assertEqual(60, maps:get(keep_alive, Data)),
...
encode_connect_protocol_header_test() ->
    Packet = mqtt_proto:encode_connect(#{client_id => <<"c">>}),
...
encode_connect_with_username_password_test() ->
    Packet = mqtt_proto:encode_connect(#{
        client_id => <<"c">>,
        username => <<"user">>,
        password => <<"pass">>
    }),
...
encode_connect_clean_session_false_test() ->
    Packet = mqtt_proto:encode_connect(#{client_id => <<"c">>, clean_session => false}),
    {ok, {connect, Data}, <<>>} = mqtt_proto:decode(Packet),
    ?assertEqual(false, maps:get(clean_session, Data)).

encode_connect_custom_keep_alive_test() ->
    Packet = mqtt_proto:encode_connect(#{client_id => <<"c">>, keep_alive => 120}),
    {ok, {connect, Data}, <<>>} = mqtt_proto:decode(Packet),
    ?assertEqual(120, maps:get(keep_alive, Data)).
...
encode_decode_publish_qos0_test() ->
    Packet = mqtt_proto:encode_publish(#{topic => <<"t">>, message => <<"m">>, qos => 0}),
    {ok, {publish, Data}, <<>>} = mqtt_proto:decode(Packet),
...
encode_decode_publish_qos1_test() ->
    Packet = mqtt_proto:encode_publish(#{
...
encode_decode_publish_empty_message_test() ->
    Packet = mqtt_proto:encode_publish(#{topic => <<"t">>, message => <<>>, qos => 0}),
    {ok, {publish, Data}, <<>>} = mqtt_proto:decode(Packet),
    ?assertEqual(<<>>, maps:get(message, Data)).

encode_decode_publish_large_message_test() ->
    BigMsg = binary:copy(<<"x">>, 200),
    Packet = mqtt_proto:encode_publish(#{topic => <<"t">>, message => BigMsg, qos => 0}),
    {ok, {publish, Data}, <<>>} = mqtt_proto:decode(Packet),
...
encode_decode_puback_test() ->
    Packet = mqtt_proto:encode_puback(7),
    ?assertEqual(<<16#40, 2, 0, 7>>, Packet),
    ?assertMatch({ok, {puback, #{packet_id := 7}}, <<>>}, mqtt_proto:decode(Packet)).

encode_decode_pubrec_test() ->
    Packet = mqtt_proto:encode_pubrec(99),
    ?assertEqual(<<16#50, 2, 0, 99>>, Packet),
    ?assertMatch({ok, {pubrec, #{packet_id := 99}}, <<>>}, mqtt_proto:decode(Packet)).

encode_decode_pubrel_reserved_flags_test() ->
    Packet = mqtt_proto:encode_pubrel(99),
    %% PUBREL type=6, flags=2 -> 0x62
    ?assertEqual(<<16#62, 2, 0, 99>>, Packet),
    ?assertMatch({ok, {pubrel, #{packet_id := 99}}, <<>>}, mqtt_proto:decode(Packet)).

encode_decode_pubcomp_test() ->
    Packet = mqtt_proto:encode_pubcomp(99),
    ?assertEqual(<<16#70, 2, 0, 99>>, Packet),
    ?assertMatch({ok, {pubcomp, #{packet_id := 99}}, <<>>}, mqtt_proto:decode(Packet)).
...
encode_decode_subscribe_single_test() ->
    Packet = mqtt_proto:encode_subscribe(#{packet_id => 1, topics => [{<<"a/b">>, 1}]}),
    {ok, {subscribe, Data}, <<>>} = mqtt_proto:decode(Packet),
    ?assertEqual(1, maps:get(packet_id, Data)),
    ?assertEqual([{<<"a/b">>, 1}], maps:get(topics, Data)).

encode_decode_subscribe_multiple_test() ->
    Packet = mqtt_proto:encode_subscribe(#{
        packet_id => 5,
        topics => [{<<"a">>, 0}, {<<"b">>, 1}, {<<"c">>, 2}]
    }),
    {ok, {subscribe, Data}, <<>>} = mqtt_proto:decode(Packet),
...
subscribe_reserved_flags_test() ->
    Packet = mqtt_proto:encode_subscribe(#{packet_id => 1, topics => [{<<"t">>, 0}]}),
...
decode_suback_test() ->
    ...
    {ok, {suback, Data}, <<>>} = mqtt_proto:decode(Packet),
    ?assertEqual(1, maps:get(packet_id, Data)),
...
encode_decode_unsubscribe_test() ->
    Packet = mqtt_proto:encode_unsubscribe(#{packet_id => 3, topics => [<<"a">>, <<"b">>]}),
    {ok, {unsubscribe, Data}, <<>>} = mqtt_proto:decode(Packet),
    ?assertEqual(3, maps:get(packet_id, Data)),
    ?assertEqual([<<"a">>, <<"b">>], maps:get(topics, Data)).

unsubscribe_reserved_flags_test() ->
    Packet = mqtt_proto:encode_unsubscribe(#{packet_id => 1, topics => [<<"t">>]}),
    <<16#A2, _Rest/binary>> = Packet,
    ok.

decode_unsuback_test() ->
    Packet = <<16#B0, 2, 0, 3>>,
    ?assertMatch({ok, {unsuback, #{packet_id := 3}}, <<>>}, mqtt_proto:decode(Packet)).
...
encode_pingreq_test() ->
    ?assertEqual(<<16#C0, 0>>, mqtt_proto:encode_pingreq()).

decode_pingresp_test() ->
    ?assertMatch({ok, {pingresp, #{}}, <<>>}, mqtt_proto:decode(<<16#D0, 0>>)).

decode_pingreq_test() ->
    ?assertMatch({ok, {pingreq, #{}}, <<>>}, mqtt_proto:decode(<<16#C0, 0>>)).
...
encode_disconnect_test() ->
    ?assertEqual(<<16#E0, 0>>, mqtt_proto:encode_disconnect()).

decode_disconnect_test() ->
    ?assertMatch({ok, {disconnect, #{}}, <<>>}, mqtt_proto:decode(<<16#E0, 0>>)).
...
decode_incomplete_empty_test() ->
    ?assertEqual({error, incomplete}, mqtt_proto:decode(<<>>)).

decode_incomplete_only_fixed_header_test() ->
    ?assertEqual({error, incomplete}, mqtt_proto:decode(<<16#20>>)).

decode_incomplete_remaining_length_says_more_test() ->
    %% CONNACK header but missing payload
    ?assertEqual({error, incomplete}, mqtt_proto:decode(<<16#20, 2, 0>>)).

decode_incomplete_variable_length_test() ->
    ?assertEqual({error, incomplete}, mqtt_proto:decode(<<16#30, 128>>)).

decode_two_packets_concatenated_test() ->
    P1 = <<16#D0, 0>>,
    P2 = <<16#D0, 0>>,
    Combined = <<P1/binary, P2/binary>>,
    {ok, {pingresp, _}, Rest} = mqtt_proto:decode(Combined),
    ?assertMatch({ok, {pingresp, _}, <<>>}, mqtt_proto:decode(Rest)).

decode_connack_with_trailing_publish_test() ->
    Connack = <<16#20, 2, 0, 0>>,
    Publish = mqtt_proto:encode_publish(#{topic => <<"t">>, message => <<"m">>, qos => 0}),
    Combined = <<Connack/binary, Publish/binary>>,
    {ok, {connack, _}, Rest} = mqtt_proto:decode(Combined),
    {ok, {publish, Data}, <<>>} = mqtt_proto:decode(Rest),
    ?assertEqual(<<"t">>, maps:get(topic, Data)).
...
decode_publish_qos3_rejected_test() ->
    ...
    ?assertMatch({error, {protocol_error, _}}, mqtt_proto:decode(Packet)).
