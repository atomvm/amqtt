...
connect_and_handshake(Port, ClientId, KeepAliveSeconds) ->
...
connect_and_receive_connack_success_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        {ok, {connect, ConnectInfo}, <<>>} = amqtt_proto:decode(ConnectData),
...
publish_qos0_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ok = amqtt_client:publish(Client, <<"topic/a">>, <<"hello">>, 0),
...
subscribe_and_receive_suback_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        SubAsync = async_call(fun() ->
            amqtt_client:subscribe(Client, [{<<"test/topic">>, 1}])
        end),

        SubData = recv_packet(BrokerSocket),
        {ok, {subscribe, SubInfo}, <<>>} = amqtt_proto:decode(SubData),
...
receive_published_message_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        PublishPacket = iolist_to_binary(
...
disconnect_sends_packet_and_stops_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ?assertMatch({ok, {disconnect, #{}}, <<>>}, amqtt_proto:decode(DisconnectData)),
...
automatic_ping_on_keep_alive_seconds_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        {Listener, Port} = start_mock_broker(),
        ...
        ?assertMatch({ok, {pingreq, #{}}, <<>>}, amqtt_proto:decode(PingData)),
...
receive_publish_qos1_auto_puback_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ?assertMatch({ok, {puback, #{packet_id := 7}}, <<>>}, amqtt_proto:decode(AckData)),
...
publish_qos2_full_flow_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        PubAsync = async_call(fun() ->
            amqtt_client:publish(Client, <<"t">>, <<"m">>, 2)
        end),
        ...
        send_broker(BrokerSocket, amqtt_proto:encode_pubrec(PacketId)),

        PubrelData = recv_packet(BrokerSocket),
        ?assertMatch(
            {ok, {pubrel, #{packet_id := PacketId}}, <<>>},
            amqtt_proto:decode(PubrelData)
        ),

        send_broker(BrokerSocket, amqtt_proto:encode_pubcomp(PacketId)),
...
receive_publish_qos2_full_flow_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ?assertMatch(
            {ok, {pubrec, #{packet_id := 10}}, <<>>}, amqtt_proto:decode(PubrecData)
        ),

        send_broker(BrokerSocket, amqtt_proto:encode_pubrel(10)),

        PubcompData = recv_packet(BrokerSocket),
        ?assertMatch(
            {ok, {pubcomp, #{packet_id := 10}}, <<>>}, amqtt_proto:decode(PubcompData)
        ),
...
unsubscribe_and_receive_unsuback_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        UnsubAsync = async_call(fun() ->
            amqtt_client:unsubscribe(Client, [<<"test/topic">>])
        end),

        UnsubData = recv_packet(BrokerSocket),
        {ok, {unsubscribe, UnsubInfo}, <<>>} = amqtt_proto:decode(UnsubData),
...
connect_with_pre_opened_gen_tcp_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ok = amqtt_client:publish(Client, <<"t">>, <<"via-custom">>, 0),
        PubData = recv_packet(BrokerSocket),
        {ok, {publish, PubInfo}, <<>>} = amqtt_proto:decode(PubData),
        ...
        send_broker(
            BrokerSocket,
            iolist_to_binary(
                amqtt_proto:encode_publish(#{topic => <<"t">>, message => <<"back">>, qos => 0})
            )
        ),
...
passive_transport_publish_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ok = amqtt_client:publish(Client, <<"t">>, <<"passive_msg">>, 0),
        PubData = recv_packet(BrokerSocket),
        {ok, {publish, PubInfo}, <<>>} = amqtt_proto:decode(PubData),
        ...
        send_broker(
            ...
            iolist_to_binary(
                amqtt_proto:encode_publish(#{
                    topic => <<"t">>, message => <<"from_broker">>, qos => 0
                })
            )
        ),
...
passive_transport_subscribe_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        SubAsync = async_call(fun() ->
            amqtt_client:subscribe(Client, [{<<"topic">>, 0}])
        end),
...
publish_before_connack_returns_not_connected_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ?assertEqual(
            {error, not_connected},
            amqtt_client:subscribe(Client, [{<<"t">>, 0}])
        ),
...
inbound_qos2_dedup_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ?assertMatch(
            ...
            amqtt_proto:decode(recv_packet(BrokerSocket))
        ),
        ...
        send_broker(BrokerSocket, amqtt_proto:encode_pubrel(99)),
        ?assertMatch(
...
manual_ack_qos1_holds_puback_until_ack_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ok = amqtt_client:ack(Client, 11),
        AckData = recv_packet(BrokerSocket),
        ?assertMatch({ok, {puback, #{packet_id := 11}}, <<>>}, amqtt_proto:decode(AckData)),
...
manual_ack_qos2_holds_pubrec_until_ack_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ok = amqtt_client:ack(Client, 22),
        ?assertMatch(
            {ok, {pubrec, #{packet_id := 22}}, <<>>},
            amqtt_proto:decode(recv_packet(BrokerSocket))
        ),

        send_broker(BrokerSocket, amqtt_proto:encode_pubrel(22)),
        ?assertMatch(
...
manual_ack_unknown_packet_id_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ?assertEqual({error, not_pending}, amqtt_client:ack(Client, 9999)),
...
manual_ack_on_auto_ack_client_errors_test_() ->
    {timeout, ?TEST_TIMEOUT, fun() ->
        ...
        ?assertEqual({error, auto_ack_enabled}, amqtt_client:ack(Client, 1)),
