...
-type qos() :: mqtt_proto:qos().
-type packet_id() :: mqtt_proto:packet_id().
-type topic() :: binary().
...
-type mqtt_event() ::
    {mqtt, pid(), connack, mqtt_proto:connack_data()}
    | {mqtt, pid(), publish, mqtt_proto:publish_data()}
...
-type inbound_entry() :: {inbound_phase(), qos(), mqtt_proto:publish_data()}.
...
-spec publish(pid(), topic(), binary(), qos()) -> ok | {ok, packet_id()} | {error, term()}.
publish(Pid, Topic, Message, QoS) ->
    publish(Pid, Topic, Message, QoS, #{}).

%% @doc Publish a message to `Topic'.
%%
%% At QoS 0 this is fire-and-forget: the call returns `ok' as soon as
%% the cast is queued, and there is no broker acknowledgment.
%%
%% At QoS 1 the call blocks until PUBACK arrives; at QoS 2 it blocks
%% through the full PUBREC/PUBREL/PUBCOMP exchange. In both cases the
%% return is `{ok, PacketId}'. If the broker never acks within the
%% server's request timeout the call returns `{error, timeout}' rather
%% than exiting.
%%
...
%% a transport error).
%% @end
-spec publish(pid(), topic(), binary(), qos(), map()) -> ok | {ok, packet_id()} | {error, term()}.
...
init(Opts) ->
    ...
    KeepAlive = maps:get(keep_alive, Opts, 60),
...
handle_call(send_connect_packet, _From, #state{pending_connect_opts = Opts} = State) ->
    ...
    Packet = mqtt_proto:encode_connect(Opts),
...
handle_call(disconnect, _From, State) ->
    _ = raw_send(mqtt_proto:encode_disconnect(), State),
...
handle_call({publish, Topic, Message, QoS, Opts}, From, State) ->
    case alloc_packet_id(State) of
        {ok, PacketId, State1} ->
            Packet = mqtt_proto:encode_publish(Opts#{
                topic => Topic,
                message => Message,
                qos => QoS,
                packet_id => PacketId
...
handle_call({subscribe, Topics}, From, State) ->
    case alloc_packet_id(State) of
        {ok, PacketId, State1} ->
            Packet = mqtt_proto:encode_subscribe(#{packet_id => PacketId, topics => Topics}),
...
handle_call({unsubscribe, Topics}, From, State) ->
    case alloc_packet_id(State) of
        {ok, PacketId, State1} ->
            Packet = mqtt_proto:encode_unsubscribe(#{packet_id => PacketId, topics => Topics}),
...
handle_call({ack, PId}, _From, #state{inbound_pending_ack = Pending} = State) ->
    case maps:find(PId, Pending) of
        {ok, {awaiting_ack, 1, _Data}} ->
            case send_packet(mqtt_proto:encode_puback(PId), State) of
                {ok, State1} ->
                    {reply, ok, State1#state{
                        inbound_pending_ack = maps:remove(PId, Pending)
                    }};
                {error, Reason, State1} ->
                    {reply, {error, Reason}, State1}
            end;
        {ok, {awaiting_ack, 2, Data}} ->
            case send_packet(mqtt_proto:encode_pubrec(PId), State) of
                {ok, State1} ->
                    {reply, ok, State1#state{
                        inbound_pending_ack = Pending#{PId => {pubrec_sent, 2, Data}}
                    }};
                {error, Reason, State1} ->
                    {reply, {error, Reason}, State1}
            end;
...
handle_cast({publish, Topic, Message, 0, Opts}, State) ->
    Packet = mqtt_proto:encode_publish(Opts#{topic => Topic, message => Message, qos => 0}),
    case send_packet(Packet, State) of
        {ok, State1} -> {noreply, State1};
        {error, Reason, State1} -> stop_with_error(Reason, State1)
    end;
handle_cast(ping, State) ->
    case send_packet(mqtt_proto:encode_pingreq(), State) of
...
handle_info(send_ping, State) ->
    case send_packet(mqtt_proto:encode_pingreq(), State#state{ping_timer = undefined}) of
        {ok, State1} -> {noreply, arm_pingresp_timer(State1)};
        {error, Reason, State1} -> stop_with_error(Reason, State1)
    end;
...
handle_packet(
    ...
    case maps:is_key(PId, Pending) of
        true ->
            {ok, State};
        false ->
            Owner ! {mqtt, self(), publish, Data},
            {ok, State#state{
                inbound_pending_ack = Pending#{PId => {awaiting_ack, 1, Data}}
            }}
    end;
handle_packet(
    publish,
    #{qos := 2, packet_id := PId} = Data,
    ...
    State1 =
        case maps:is_key(PId, Set) of
            true ->
                State;
            false ->
                Owner ! {mqtt, self(), publish, Data},
                State#state{inbound_qos2 = Set#{PId => true}}
        end,
    case send_packet(mqtt_proto:encode_pubrec(PId), State1) of
        {ok, State2} -> {ok, State2};
...
handle_packet(
    ...
    case maps:find(PId, Pending) of
        error ->
            Owner ! {mqtt, self(), publish, Data},
            {ok, State#state{
                inbound_qos2 = Set#{PId => true},
                inbound_pending_ack = Pending#{PId => {awaiting_ack, 2, Data}}
            }};
        {ok, {awaiting_ack, 2, _}} ->
            {ok, State};
        {ok, {pubrec_sent, 2, _}} ->
...
handle_packet(puback, #{packet_id := PId}, State) ->
    {ok, complete_pending(PId, puback, {ok, PId}, State)};
handle_packet(pubrec, #{packet_id := PId}, State) ->
    case send_packet(mqtt_proto:encode_pubrel(PId), State) of
        {ok, State1} ->
            case maps:get(PId, State1#state.pending, undefined) of
                {pubrec, From, TRef} ->
                    Pending = maps:put(PId, {pubcomp, From, TRef}, State1#state.pending),
...
handle_packet(
    ...
    State1 = State#state{
        inbound_qos2 = maps:remove(PId, Set),
        inbound_pending_ack = maps:remove(PId, Pending)
    },
    case send_packet(mqtt_proto:encode_pubcomp(PId), State1) of
        {ok, State2} -> {ok, State2};
        {error, Reason, State2} -> {error, Reason, State2}
    end;
handle_packet(pubcomp, #{packet_id := PId}, State) ->
    {ok, complete_pending(PId, pubcomp, {ok, PId}, State)};
handle_packet(suback, #{packet_id := PId, return_codes := RCs}, State) ->
    {ok, complete_pending(PId, suback, {ok, RCs}, State)};
handle_packet(unsuback, #{packet_id := PId}, State) ->
    {ok, complete_pending(PId, unsuback, ok, State)};
...
restart_ping_timer(#state{keep_alive = 0} = State) ->
    State;
restart_ping_timer(State) ->
    State1 = cancel_ping_timer(State),
    Interval = State1#state.keep_alive * 750,
...
arm_pingresp_timer(#state{keep_alive = 0} = State) ->
    State;
arm_pingresp_timer(#state{pingresp_timer = OldRef, keep_alive = KA} = State) ->
    case OldRef of
        undefined -> ok;
