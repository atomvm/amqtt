...
%% take their MQTT 3.1.1 defaults (`keep_alive => 60',
%% `clean_session => true', no will, no credentials).
%% @end
-spec encode_connect(connect_opts()) -> binary().
encode_connect(Opts) ->
    ...
    VarHeader =
        <<0, 4, "MQTT", 4, UsernameBit:1, PasswordBit:1, WillRetainBit:1, WillQoSBits:2, WillFlag:1,
            CleanSessionBit:1, 0:1, KeepAlive:16/big>>,

    Payload0 = encode_utf8(ClientId),
    Payload1 =
        case HasWill of
            true ->
                <<Payload0/binary, (encode_utf8(WillTopic))/binary,
                    (encode_utf8(WillMessage))/binary>>;
            false ->
                Payload0
        end,
    Payload2 =
        case HasUsername of
            true -> <<Payload1/binary, (encode_utf8(Username))/binary>>;
            false -> Payload1
        end,
    Payload3 =
        case HasPassword of
            true -> <<Payload2/binary, (encode_utf8(Password))/binary>>;
            false -> Payload2
        end,

    Body = <<VarHeader/binary, Payload3/binary>>,
    RemainingLen = encode_remaining_length(byte_size(Body)),
    <<?CONNECT:4, 0:4, RemainingLen/binary, Body/binary>>.
...
%% `false'.
%% @end
-spec encode_publish(publish_opts()) -> binary().
encode_publish(Opts) ->
    ...
    VarHeader =
        case QoS of
            ...
            _ ->
                ...
                <<TopicBin/binary, PacketId:16/big>>
        end,

    Body = <<VarHeader/binary, Message/binary>>,
    RemainingLen = encode_remaining_length(byte_size(Body)),
    <<?PUBLISH:4, DupBit:1, QoS:2, RetainBit:1, RemainingLen/binary, Body/binary>>.
...
%% @end
-spec encode_subscribe(subscribe_opts()) -> binary().
encode_subscribe(Opts) ->
    PacketId = maps:get(packet_id, Opts),
    Topics = maps:get(topics, Opts),

    Payload = encode_subscribe_topics(Topics, <<>>),
    Body = <<PacketId:16/big, Payload/binary>>,
    RemainingLen = encode_remaining_length(byte_size(Body)),
    <<?SUBSCRIBE:4, 2:4, RemainingLen/binary, Body/binary>>.

encode_subscribe_topics([], Acc) ->
    Acc;
encode_subscribe_topics([{Topic, QoS} | Rest], Acc) ->
    TopicBin = encode_utf8(Topic),
    encode_subscribe_topics(Rest, <<Acc/binary, TopicBin/binary, QoS>>).
...
%% topic filters.
%% @end
-spec encode_unsubscribe(unsubscribe_opts()) -> binary().
encode_unsubscribe(Opts) ->
    PacketId = maps:get(packet_id, Opts),
    Topics = maps:get(topics, Opts),

    Payload = encode_unsubscribe_topics(Topics, <<>>),
    Body = <<PacketId:16/big, Payload/binary>>,
    RemainingLen = encode_remaining_length(byte_size(Body)),
    <<?UNSUBSCRIBE:4, 2:4, RemainingLen/binary, Body/binary>>.

encode_unsubscribe_topics([], Acc) ->
    Acc;
encode_unsubscribe_topics([Topic | Rest], Acc) ->
    TopicBin = encode_utf8(Topic),
    encode_unsubscribe_topics(Rest, <<Acc/binary, TopicBin/binary>>).
